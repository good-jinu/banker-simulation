...
export const MAX_WAIT_DAYS = 1440;
export const MAX_SCHEDULE_OCCURRENCES = 24;
...
export function flattenContractSteps(
  ...
  return steps.flatMap((step) =>
    step.type === "if"
      ...
      : step.type === "schedule"
        ? [step, ...flattenContractSteps(step.steps)]
        : [step],
  );
}

function validateScheduledProgram(
  ...
  const forbidden = program.steps.filter((step) =>
    ["if", "collateral", "collect"].includes(step.type),
  );
  for (const step of forbidden)
    issues.push(
      issue(
        "schedule-mixed-flow",
        "error",
        "A scheduled agreement cannot mix a root Collect, recovery, or branch block.",
        step.id,
      ),
    );
  if (program.steps.filter((step) => step.type === "schedule").length !== 1)
    issues.push(
      issue(
        "duplicate-schedule",
        "error",
        "Use one payment schedule.",
        schedule.id,
      ),
    );
  const lend = program.steps.find(
    (step): step is LendStep => step.type === "lend",
  );
  const wait = program.steps.find(
    (step): step is WaitStep => step.type === "wait",
  );
  const close = program.steps.find((step) => step.type === "close");
  if (!lend) issues.push(issue("missing-lend", "error", "Add a Lend block."));
  if (!wait) issues.push(issue("missing-wait", "error", "Add a Wait block."));
  if (!close)
    issues.push(issue("missing-close", "error", "Add a Close block."));
  const order = [lend, wait, schedule, close];
  if (
    order.some((step) => !step) ||
    !(
      program.steps.indexOf(lend!) < program.steps.indexOf(wait!) &&
      program.steps.indexOf(wait!) < program.steps.indexOf(schedule) &&
      program.steps.indexOf(schedule) < program.steps.indexOf(close!)
    )
  )
    issues.push(
      issue(
        "schedule-order",
        "error",
        "A schedule reads Lend → Wait → Schedule → Close.",
        schedule.id,
      ),
    );
  if (!Number.isInteger(schedule.intervalDays) || schedule.intervalDays < 1)
    issues.push(
      issue(
        "schedule-interval",
        "error",
        "Choose a positive interval in days.",
        schedule.id,
      ),
    );
  if (
    !Number.isInteger(schedule.occurrences) ||
    schedule.occurrences < 1 ||
    schedule.occurrences > MAX_SCHEDULE_OCCURRENCES
  )
    issues.push(
      issue(
        "schedule-occurrences",
        "error",
        `Schedule between 1 and ${MAX_SCHEDULE_OCCURRENCES} payments.`,
        schedule.id,
      ),
    );
  if (
    wait &&
    schedule.intervalDays * schedule.occurrences + wait.days > MAX_WAIT_DAYS
  )
    issues.push(
      issue(
        "schedule-duration",
        "error",
        `The final payment must be within ${MAX_WAIT_DAYS} days.`,
        schedule.id,
      ),
    );
  if (schedule.steps.length !== 1 || schedule.steps[0]?.type !== "collect")
    issues.push(
      issue(
        "schedule-body",
        "error",
        "A schedule contains exactly one Collect block.",
        schedule.id,
      ),
    );
  const collect = schedule.steps[0];
  if (collect?.type === "collect" && lend) {
    if (collect.fromId !== lend.borrowerId)
      issues.push(
        issue(
          "party",
          "error",
          "Collect from the funded borrower.",
          collect.id,
        ),
      );
    if (collect.currency !== lend.currency)
      issues.push(
        issue("currency", "error", "Use the lending currency.", collect.id),
      );
    if (!Number.isSafeInteger(collect.amount) || collect.amount <= 0)
      issues.push(
        issue(
          "collect-amount",
          "error",
    ...
    if (collect.amount * schedule.occurrences < lend.amount)
      issues.push(
        issue(
          "negative-return",
          "warning",
          "This schedule returns less than it lends.",
          collect.id,
        ),
      );
  }
...
export function validateProgram(program: ContractProgram): ValidationIssue[] {
  ...
  if (program.steps.some((step) => step.type === "schedule"))
    return validateScheduledProgram(program, issues);
  ...
  if (
    wait &&
    (!Number.isInteger(wait.days) || wait.days < 1 || wait.days > MAX_WAIT_DAYS)
  ) {
    issues.push(
      issue(
        "wait-duration",
        "error",
        `Wait between 1 and ${MAX_WAIT_DAYS} days.`,
        wait.id,
      ),
    );
  }
